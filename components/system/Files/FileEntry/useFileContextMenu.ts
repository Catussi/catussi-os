    ...
    () =>
      contextMenu?.(() => {
        ...
        if (!readOnly && !remoteMount) {
          ...
          menuItems.push(
            { action: () => moveEntries(absoluteEntries()), label: ui.cut },
            { action: () => copyEntries(absoluteEntries()), label: ui.copy },
            MENU_SEPERATOR
          );
          ...
          menuItems.push(
            ...
            { action: () => setRenaming(baseName), label: ui.rename },
            MENU_SEPERATOR,
            {
              action: () => {
